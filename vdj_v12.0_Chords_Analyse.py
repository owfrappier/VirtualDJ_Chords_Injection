...
# ============================================================
# VDJ CHORD INJECTOR V11.5 ULTRA FAST SAFE - PYTHON ENGINE
# - NO Song deletion
# - NO duplicate cleanup / merge
# - patches only ONE full existing Song block
# - indexed prescan / indexed patch lookup
# - V10 7th arbitration / anti fake maj7
# - V11 faster DB patching: no full index rebuild after every patch
# - DJ SAFE fine pitch detector
# ============================================================
...
def auto_max_workers():
    """CPU-aware worker count. Override with env VDJ_MAX_WORKERS if needed."""
    try:
        import os
...
def auto_write_every():
    """Batch DB writes. Override with env VDJ_WRITE_EVERY. 1 = old ultra-safe behavior."""
    try:
        import os
...
def is_full_song_block(block):
    return '<Infos ' in block and '<Scan ' in block
...
def build_song_index(text):
    """Index DB once: filename key -> list of (start, end, block). No deletion."""
    index = {}
    total_blocks = 0
    patchable_blocks = 0
...
def remove_old_chord_pois(lines):
    ...
    for line in lines:
        if '<Poi ' in line:
            ...
            if typ == 'action' and ('pitch_zero' in action or name.startswith('pitch') or '%' in name):
                continue

            if ('Color=' + q + COLOR + q) in line and ('Type=' + q + 'action' + q) in line:
                continue
...
def detect_fine_pitch_cents(y, sr):
    try:
        ...
        final = float(np.average(cents_vals[good], weights=conf_vals[good]))
        return final
...
def analyze_wrapper(audio):
    try:
        return (str(audio), analyze_chords(audio), None)
    except Exception as e:
        return (str(audio), [], str(e))


def load_audio_mono(audio_path, sr_target):
    """Fast path for AIFF/WAV/FLAC via soundfile, fallback to librosa for compressed formats."""
    try:
        import numpy as np
...
def analyze_chords(audio_path):
    ...
    EARLY_CHORD_LIMIT_SEC = 2.0
    EARLY_CHORD_ADVANCE_SEC = 0.10
    ...
    try:
        chroma_fast = librosa.feature.chroma_cqt(
            y=y_harm, sr=sr, hop_length=HOP,
            bins_per_octave=BINS_PER_OCTAVE,
            n_chroma=12,
            fmin=librosa.note_to_hz(CHROMA_FMIN)
        )
        bass_chroma = librosa.feature.chroma_cqt(
            y=y_harm, sr=sr, hop_length=HOP,
            bins_per_octave=BINS_PER_OCTAVE,
            n_chroma=12,
            fmin=librosa.note_to_hz(BASS_FMIN)
        )
    except Exception as e:
    ...
    for i in range(chroma_slow.shape[1]):
        ...
        for root in range(12):
            for suffix, intervals, weight in chord_templates:
                root_e = float(vec[root])
                maj3_e = float(vec[(root + 4) % 12])
                min3_e = float(vec[(root + 3) % 12])
                fifth_e = float(vec[(root + 7) % 12])
                dim5_e = float(vec[(root + 6) % 12])
                aug5_e = float(vec[(root + 8) % 12])
                dom7_e = float(vec[(root + 10) % 12])
                maj7_e = float(vec[(root + 11) % 12])

                # Si la tierce n'est pas assez présente, on évite majeur/mineur inventé.
                if suffix == '':
                    if maj3_e < root_e * THIRD_MIN_RATIO:
                        continue

                if suffix == 'm':
                    if min3_e < root_e * THIRD_MIN_RATIO:
                        continue
                ...
                if suffix == '7':
                    if maj3_e < root_e * THIRD_MIN_RATIO:
                        continue
                    if dom7_e < max(root_e, maj3_e) * DOM7_REQUIRED_RATIO:
                        continue

                if suffix == 'm7':
                    if min3_e < root_e * THIRD_MIN_RATIO:
                        continue
                    if dom7_e < max(root_e, min3_e) * M7_REQUIRED_RATIO:
                        continue
                ...
                if suffix == 'dim':
                    if min3_e < root_e * THIRD_MIN_RATIO:
                        continue
                    if dim5_e < max(root_e, min3_e) * DIM_REQUIRED_RATIO:
                        continue
                    if fifth_e > dim5_e * 0.95:
                        continue
                ...
                if suffix == 'dim7':
                    ...
                    if dim5_e < max(root_e, min3_e) * DIM_REQUIRED_RATIO:
                        continue
                    if dim7_e < max(root_e, min3_e) * 0.40:
                        continue
                    if fifth_e > dim5_e * 0.95:
                        continue
                ...
                if suffix == 'aug':
                    if maj3_e < root_e * THIRD_MIN_RATIO:
                        continue
                    if aug5_e < max(root_e, maj3_e) * AUG_REQUIRED_RATIO:
                        continue
                    if fifth_e > aug5_e * 0.95:
                        continue
                ...
                tmpl[root] += ROOT_BONUS
                tmpl[(root + 7) % 12] += FIFTH_BONUS

                if suffix.startswith('m'):
                    tmpl[(root + 3) % 12] += THIRD_BONUS
                elif suffix in ('', '7', 'maj7', '6', 'aug'):
                    tmpl[(root + 4) % 12] += THIRD_BONUS
        ...
        if best_suffix == 'maj7':
            ...
            for cand in candidates:
                ...
                if r2 == best_root and suf2 in ('', 'm', '5') and s2 >= best_score - MAJ7_TRIAD_MARGIN:
                    if fallback is None or s2 > fallback[0]:
                        fallback = cand
        ...
        bass_is_valid = (
            inversion_allowed
            and bass_pc != best_root
            and bass_pc in chord_pcs
            and bass_strength >= BASS_MIN_STRENGTH
            and bass_strength >= root_strength * BASS_ROOT_RATIO
            and not bass_isolated
        )

        if bass_is_valid:
            labels.append(best_name + '/' + note_name(bass_pc))
        else:
    ...
    for i, label in enumerate(labels):
        if label != current:
            if current is not None and start_i is not None:
                ...
                dur = end_t - start_t
                avg_score = float(np.mean(seg_scores)) if seg_scores else 0.0
                if accept_segment(start_t, dur, avg_score):
                    t = place_chord_time(start_t)
                    results.append((t, current, round(avg_score, 3)))
    ...
    for t, name, score in results:
        ...
        if 'maj7' in name and (t - prev_t) < MICRO_MAJ7_SEC:
            continue

        if abs(prev_t - t) < 0.05:
            if score > prev_score:
                cleaned[-1] = (t, name, score)
            continue

        if t - prev_t < MIN_CHANGE_GAP:
            if base_chord(name) == base_chord(prev_name):
                continue
            if score < PASSING_CHORD_MIN_SCORE:
                continue
    ...
    vdj_pitch = -cents * VDJ_PITCH_FACTOR
    pitch_action = VDJ_PITCH_ACTION_PREFIX + format(vdj_pitch, '+.2f') + '%'
    pitch_poi = [(AUTO_PITCH_POI_POS, pitch_action, 1.0)]

    return pitch_poi + cleaned
...
def patch_song_block(block, chords):
    ...
    for item in chords:
        ...
        # On garde les accords avant fadeStart.
        # On filtre seulement après fadeEnd.
        if fade_end is not None and pos >= fade_end:
            continue

        filtered.append((pos, chord, score))
        
    # === GARDER LE 1er ACCORD, MAIS LE PLACER JUSTE APRÈS LE POI PITCH ===
    ...
    for pos, chord, score in filtered:
        ...
        if not first_chord_done:
            # On garde le premier accord analysé, mais on le décale juste après le pitch.
            pos = max(pos, first_allowed_chord_pos)
            first_chord_done = True
            new_filtered.append((pos, chord, score))
            continue

    # On supprime uniquement les accords trop proches après le premier,
    # pour éviter une accumulation visuelle au début.
        if pos < first_allowed_chord_pos:
            continue
...
def write_cache(rows):
    ...
    root.set('Version', 'V11.5-ULTRA-FAST-SAFE')
    ...
    for r in rows:
        ...
        if not r['matched']:
            tr.set('Warning', 'UNMATCHED_NOT_IN_VDJ_DATABASE')
        tr.set('Count', str(len(r['chords'])))
        if r.get('error'):
            tr.set('Error', r['error'])
        for item in r['chords']:
            ...
            c.set('Name', str(chord))
            c.set('Pos', format(float(pos), '.3f'))
            if score is not None:
                c.set('Score', format(float(score), '.3f'))
...
def count_chord_like_pois(block):
    ...
    for line in block.splitlines():
        if '<Poi ' not in line:
            continue
        if is_chord_name(get_xml_attr(line, 'Name')):
            c += 1
    return c
...
def main():
    ...
    with open(LOG_PATH, 'w', encoding='utf-8') as f:
        print('--- VDJ CHORD ANALYSIS V11.5 ULTRA FAST SAFE START ---', file=f)
    ...
    if SEARCH_WORD:
        audio_files = [p for p in audio_files if SEARCH_WORD in p.name.lower()]
    if FIRST_LETTER:
        audio_files = [p for p in audio_files if p.name.lower().startswith(FIRST_LETTER)]
    if not audio_files:
        ...
        for prescan_idx, audio in enumerate(audio_files, start=1):
            ...
            for _, _, block in sorted(matches, key=lambda item: song_block_score(item[2]), reverse=True):
                if has_existing_chords(block):
                    already_has_chords = True
                    break

            if already_has_chords:
                skipped_before += 1
            else:
    ...
    def flush_database(message):
        ...
        tmp = Path(str(DB_PATH) + '.tmp-v11-safe')
    ...
    def patch_one_track_in_db(current_text, current_index, audio_path, chords):
        ...
        if not matches:
            return current_text, current_index, False, 'NOT_FOUND', False
        ...
        # Fast local index update: update current key and shift offsets after patch.
        # If something unexpected happens, main can rebuild the full index later.
        updated_index = {}
    ...
    try:
        with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = [executor.submit(analyze_wrapper, a) for a in audio_files]
            completed = 0
                ...
                try:
                    ...
                    if patched:
                        ...
                        if pending_patches >= WRITE_EVERY:
                            ...
                            pending_patches = 0
                            if pending_reindex:
                                song_index, _, _ = build_song_index(new_text)
                                pending_reindex = False
                    else:
